 ...
 */
class HTML extends AbstractWriter implements IWriter
{
    /**
     * PHPExcel object
     *
    ...
    public function generateHTMLHeader($pIncludeStyles = false)
    {
        ...
        if (is_null($this->phpExcel)) {
            throw new WriterException('Internal PHPExcel object not set to an instance of an object.');
        }
        ...
        foreach ($pSheet->getChartCollection() as $chart) {
            if ($chart instanceof Chart) {
                ...
                if ($chartCoordinates['cell'] == $coordinates) {
                    $chartFileName = File::sys_get_temp_dir().'/'.uniqid().'.png';
    ...
    public function buildCSS($generateSurroundingHTML = true)
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            foreach ($sheet->getColumnDimensions() as $columnDimension) {
                if (($width = Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->defaultFont)) >= 0) {
                    $width = Drawing::pixelsToPoints($width);
    ...
    /**
     * Create CSS style (Alignment)
     *
     * @param    Alignment        $pStyle            Alignment
     * @return    array
     */
...
//        $css = $this->mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
     ...
     * @throws    Exception
     */
    private function generateRow(Worksheet $pSheet, $pValues = null, $pRow = 0, $cellType = 'td')
    {
        if (is_array($pValues)) {
            // Construct HTML
            $html = '';
            ...
            foreach ($pValues as $cellAddress) {
                ...
                if ($writeCell) {
                    ...
                    if (!$this->useInlineCss) {
                        ...
                        //** Necessary redundant code for the sake of PDF **
