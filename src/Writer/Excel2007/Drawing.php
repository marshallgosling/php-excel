...
use Nathan\PHPExcel\Shared\Drawing as SharedDrawing;
...
class Drawing extends WriterPart
{
     ...
     * @throws     WriterException
     */
    public function writeChart(XMLWriter $objWriter = null, Chart $pChart = null, $pRelationId = -1)
    {
        ...
        $objWriter->startElement('xdr:twoCellAnchor');
            ...
            $objWriter->startElement('xdr:from');
                ...
                $objWriter->writeElement('xdr:colOff', SharedDrawing::pixelsToEMU($tl['xOffset']));
                $objWriter->writeElement('xdr:row', $tl['colRow'][1] - 1);
                $objWriter->writeElement('xdr:rowOff', SharedDrawing::pixelsToEMU($tl['yOffset']));
            $objWriter->endElement();
            $objWriter->startElement('xdr:to');
                $objWriter->writeElement('xdr:col', Cell::columnIndexFromString($br['colRow'][0]) - 1);
                $objWriter->writeElement('xdr:colOff', SharedDrawing::pixelsToEMU($br['xOffset']));
                $objWriter->writeElement('xdr:row', $br['colRow'][1] - 1);
                $objWriter->writeElement('xdr:rowOff', SharedDrawing::pixelsToEMU($br['yOffset']));
            $objWriter->endElement();
     ...
     * @throws     WriterException
     */
    public function writeDrawing(XMLWriter $objWriter = null, BaseDrawing $pDrawing = null, $pRelationId = -1)
    {
        if ($pRelationId >= 0) {
            // xdr:oneCellAnchor
            ...
            $objWriter->writeElement('xdr:colOff', SharedDrawing::pixelsToEMU($pDrawing->getOffsetX()));
            $objWriter->writeElement('xdr:row', $aCoordinates[1] - 1);
            $objWriter->writeElement('xdr:rowOff', SharedDrawing::pixelsToEMU($pDrawing->getOffsetY()));
            $objWriter->endElement();

            // xdr:ext
            $objWriter->startElement('xdr:ext');
            $objWriter->writeAttribute('cx', SharedDrawing::pixelsToEMU($pDrawing->getWidth()));
            $objWriter->writeAttribute('cy', SharedDrawing::pixelsToEMU($pDrawing->getHeight()));
            $objWriter->endElement();
            ...
            $objWriter->writeAttribute('rot', SharedDrawing::degreesToAngle($pDrawing->getRotation()));
            $objWriter->endElement();
...
*/
            ...
            if ($pDrawing->getShadow()->getVisible()) {
                ...
                $objWriter->writeAttribute('blurRad', SharedDrawing::pixelsToEMU($pDrawing->getShadow()->getBlurRadius()));
                $objWriter->writeAttribute('dist', SharedDrawing::pixelsToEMU($pDrawing->getShadow()->getDistance()));
                $objWriter->writeAttribute('dir', SharedDrawing::degreesToAngle($pDrawing->getShadow()->getDirection()));
...
*/
     ...
     * @throws     WriterException
     */
    private function writeVMLHeaderFooterImage(XMLWriter $objWriter = null, $pReference = '', HeaderFooterDrawing $pImage = null)
    {
        // Calculate object id
     ...
     * @return     Drawing[]        All drawings in PHPExcel
     * @throws     WriterException
     */
