...
namespace Nathan\PHPExcel\Writer;
...
use Nathan\PHPExcel\Shared\Escher;
use Nathan\PHPExcel\Shared\Escher\DgContainer;
use Nathan\PHPExcel\Shared\Escher\DgContainer\SpgrContainer;
use Nathan\PHPExcel\Shared\Escher\DgContainer\SpgrContainer\SpContainer;
use Nathan\PHPExcel\Shared\Escher\DggContainer;
use Nathan\PHPExcel\Shared\Escher\DggContainer\BstoreContainer;
use Nathan\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE;
use Nathan\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE\Blip;
use Nathan\PHPExcel\Shared\Excel5 as SharedExcel5;
use Nathan\PHPExcel\Shared\File;
use Nathan\PHPExcel\Shared\Font;
use Nathan\PHPExcel\Shared\OLE;
...
use Nathan\PHPExcel\Worksheet\BaseDrawing;
use Nathan\PHPExcel\Worksheet\Drawing as WorksheetDrawing;
use Nathan\PHPExcel\Worksheet\MemoryDrawing as WorksheetMemoryDrawing;
 ...
 */
class Excel5 extends AbstractWriter implements IWriter
{
    /**
     * PHPExcel object
     *
     ...
     * @throws    _Exception    when directory does not exist
     * @return _Excel5
     */
    ...
    private function buildWorksheetEschers()
    {
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $twoAnchor = SharedExcel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
    ...
    private function buildWorkbookEscher()
    {
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            foreach ($sheet->getDrawingCollection() as $drawing) {
                if ($drawing instanceof WorksheetDrawing) {
                    ...
                    switch ($imageFormat) {
                        ...
                        case 6: // Windows DIB (BMP), we convert to PNG
                            $blipType = BSE::BLIPTYPE_PNG;
                            ob_start();
                            imagepng(Drawing::imagecreatefrombmp($filename));
                            $blipData = ob_get_contents();
                            ob_end_clean();
                            break;
                        default:
                ...
                } elseif ($drawing instanceof WorksheetMemoryDrawing) {
                    switch ($drawing->getRenderingFunction()) {
                        case WorksheetMemoryDrawing::RENDERING_JPEG:
                            $blipType = BSE::BLIPTYPE_JPEG;
                            $renderingFunction = 'imagejpeg';
                            break;
                        case WorksheetMemoryDrawing::RENDERING_GIF:
                        case WorksheetMemoryDrawing::RENDERING_PNG:
                        case WorksheetMemoryDrawing::RENDERING_DEFAULT:
                            $blipType = BSE::BLIPTYPE_PNG;
    ...
    private function writeSummaryInformation()
    {
        ...
        if ($this->phpExcel->getProperties()->getCreated()) {
            ...
            $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
                                   ...
                                   'data'    => array('data' => OLE::LocalDate2OLE($dataProp)));
            $dataSection_NumProps++;
        }
        //    Modified Date/Time
