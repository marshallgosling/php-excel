...
namespace Nathan\PHPExcel\Reader;

use Nathan\PHPExcel\PHPExcel;
use Nathan\PHPExcel\Cell;
use Nathan\PHPExcel\DocumentProperties;
use Nathan\PHPExcel\NamedRange;
use Nathan\PHPExcel\ReferenceHelper;
use Nathan\PHPExcel\RichText;
use Nathan\PHPExcel\Settings;
use Nathan\PHPExcel\Shared\Date;
use Nathan\PHPExcel\Shared\Drawing;
use Nathan\PHPExcel\Shared\File as SharedFile;
...
use Nathan\PHPExcel\Style\Color;
use Nathan\PHPExcel\Style\Conditional;
use Nathan\PHPExcel\Style\Font;
use Nathan\PHPExcel\Style\NumberFormat;
use Nathan\PHPExcel\Style\Protection;
use Nathan\PHPExcel\Worksheet;
use Nathan\PHPExcel\Worksheet\AutoFilter\Column;
use Nathan\PHPExcel\Worksheet\AutoFilter\Column\Rule;
use Nathan\PHPExcel\Worksheet\Drawing as WorksheetDrawing;
use Nathan\PHPExcel\Worksheet\HeaderFooterDrawing;
 ...
 */
class Excel2007 extends AbstractReader implements IReader
{
    /**
    ...
    /**
     * Create a new Excel2007 instance
     */
    ...
    public function canRead($pFilename)
    {
        ...
        if (!file_exists($pFilename)) {
            throw new ReaderException("Could not open " . $pFilename . " for reading! File does not exist.");
        }
...
//        }
        ...
        if ($zip->open($pFilename) === true) {
            ...
            $rels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "_rels/.rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
            if ($rels !== false) {
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        $rels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "_rels/.rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
        foreach ($rels->Relationship as $rel) {
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                $dir = dirname($rel["Target"]);
                $relsWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/package/2006/relationships");
                ...
                $xmlWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                if ($xmlWorkbook->sheets) {
                    $dir = dirname($rel["Target"]);
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        $tmpInfo = array(
                        ...
                        $xml = new \XMLReader();
                        $res = $xml->xml($this->securityScanFile('zip://'.SharedFile::realpath($pFilename).'#'."$dir/$fileWorksheet"), null, Settings::getLibXmlLoaderOptions());
...
//                echo 'Adjusted Formula is ', $value, PHP_EOL;
    ...
    public function load($pFilename)
    {
        ...
        $wbRels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "xl/_rels/workbook.xml.rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
        foreach ($wbRels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                    ...
                    $xmlTheme = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "xl/{$rel['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
                    $xmlCore = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    $xmlStrings = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                    if (isset($xmlStrings) && isset($xmlStrings->si)) {
                        foreach ($xmlStrings->si as $val) {
                            if (isset($val->t)) {
                                $sharedStrings[] = SharedString::ControlCharacterOOXML2PHP((string) $val->t);
                            } elseif (isset($val->r)) {
                    ...
                    $xmlStyles = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            $fileWorksheet = $worksheets[(string) self::getArrayItem($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
                            $xmlSheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$fileWorksheet")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                            ...
                            if (isset($xmlSheet->cols) && !$this->readDataOnly) {
                                foreach ($xmlSheet->cols->col as $col) {
                                    for ($i = intval($col["min"]) - 1; $i < intval($col["max"]); ++$i) {
                                        ...
                                        // echo Cell::stringFromColumnIndex($i), ': HIDDEN COLUMN',PHP_EOL;
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                            ...
                            if (!$this->readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                foreach ($xmlSheet->conditionalFormatting as $conditional) {
                                    foreach ($conditional->cfRule as $cfRule) {
                                        if (((string)$cfRule["type"] == Conditional::CONDITION_NONE || (string)$cfRule["type"] == Conditional::CONDITION_CELLIS || (string)$cfRule["type"] == Conditional::CONDITION_CONTAINSTEXT || (string)$cfRule["type"] == Conditional::CONDITION_EXPRESSION) && isset($dxfs[intval($cfRule["dxfId"])])) {
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                    $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                foreach ($comments as $relName => $relPath) {
                                    ...
                                    $commentsFile = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $relPath)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    $relPath = SharedFile::realpath(dirname("$dir/$fileWorksheet") . "/" . $relPath);
                                    $vmlCommentsFile = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $relPath)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                                ...
                                if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->readDataOnly) {
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            // Fetch linked images
                                            $relsVML = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($vmlRelationship) . '/_rels/' . basename($vmlRelationship) . '.rels')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                            $drawings = array();
                                            ...
                                            // Fetch VML document
                                            $vmlDrawing = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $vmlRelationship)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                            ...
                            if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                ...
                                if ($xmlSheet->drawing && !$this->readDataOnly) {
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        $relsDrawing = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($fileDrawing) . "/_rels/" . basename($fileDrawing) . ".rels")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                        $images = array();
                                        ...
                                        $xmlDrawing = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $fileDrawing)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions())->children("http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing");
        ...
        if (!$this->readDataOnly) {
            $contentTypes = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "[Content_Types].xml")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
            foreach ($contentTypes->Override as $contentType) {
                switch ($contentType["ContentType"]) {
                    case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
                        if ($this->includeCharts) {
                            $chartEntryRef = ltrim($contentType['PartName'], '/');
                            $chartElements = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $chartEntryRef)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
                            $objChart = Excel2007\Chart::readChart($chartElements, basename($chartEntryRef, '.xml'));
...
//        }
    ...
    private function parseRichText($is = null)
    {
        ...
        if (isset($is->t)) {
            $value->createText(SharedString::ControlCharacterOOXML2PHP((string) $is->t));
        } else {
            if (is_object($is->r)) {
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        $objText = $value->createText(SharedString::ControlCharacterOOXML2PHP((string) $run->t));

                    } else {
                        $objText = $value->createTextRun(SharedString::ControlCharacterOOXML2PHP((string) $run->t));
    ...
    private function readRibbon($excel, $customUITarget, $zip)
    {
        ...
        if ($dataRels) {
            ...
            $UIRels = simplexml_load_string($this->securityScan($dataRels), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions());
            if ($UIRels) {
    ...
    private static function toCSSArray($style)
    {
        ...
        foreach ($temp as $item) {
            ...
            if (strpos($item[1], 'pt') !== false) {
                ...
                $item[1] = SharedFont::fontSizeToPixels($item[1]);
            }
            if (strpos($item[1], 'in') !== false) {
                $item[1] = str_replace('in', '', $item[1]);
                $item[1] = SharedFont::inchSizeToPixels($item[1]);
            }
            if (strpos($item[1], 'cm') !== false) {
                $item[1] = str_replace('cm', '', $item[1]);
                $item[1] = SharedFont::centimeterSizeToPixels($item[1]);
            }
