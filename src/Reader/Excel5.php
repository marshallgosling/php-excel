...
use Nathan\PHPExcel\Shared\CodePage;
...
use Nathan\PHPExcel\Shared\OLERead;
...
use Nathan\PHPExcel\Worksheet\Drawing;
use Nathan\PHPExcel\Worksheet\MemoryDrawing;
...
class Excel5 extends AbstractReader implements IReader
{
    // ParseXL definitions
...
//                echo '<br />';
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    $width = SharedExcel5::getDistanceX($this->phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
                    $height = SharedExcel5::getDistanceY($this->phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

                    // calculate offsetX and offsetY of the shape
                    $offsetX = $startOffsetX * SharedExcel5::sizeCol($this->phpSheet, $startColumn) / 1024;
                    $offsetY = $startOffsetY * SharedExcel5::sizeRow($this->phpSheet, $startRow) / 256;
            ...
            if ($definedName['isBuiltInName']) {
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->phpExcel->getSheetByName($explodes[0])) ||
                        ...
                        $this->phpExcel->addNamedRange(new NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope));
                    }
                } else {
                    //    Named Value
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x1E: // null-terminated string prepended by dword string length
                    ...
                    $value = SharedString::ConvertEncoding($value, 'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (($fmtRuns = $this->sst[$index]['fmtRuns']) && !$this->readDataOnly) {
                ...
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text = Shared_String::Substring($this->sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
                        $charPos = $fmtRuns[$i]['charPos'];
                    } else {
                        $text = Shared_String::Substring($this->sst[$index]['value'], $charPos, Shared_String::CountCharacters($this->sst[$index]['value']));
                    }
    ...
    private function readFormula()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord($recordData{6}) == 0) && (ord($recordData{12}) == 255) && (ord($recordData{13}) == 255)) {
                ...
                $value = Excel5_ErrorCode::lookup(ord($recordData{8}));
...
//        echo '<br />';
    ...
    private static function encodeUTF16($string, $compressed = '')
    {
        ...
        return SharedString::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
    }
    ...
    {
        return SharedString::ConvertEncoding($string, 'UTF-8', $this->codepage);
    }
