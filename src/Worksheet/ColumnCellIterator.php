...
class ColumnCellIterator extends CellIterator implements \Iterator
{
    ...
    public function seek($row = 1)
    {
        if (($row < $this->startRow) || ($row > $this->endRow)) {
            throw new Exception("Row $row is out of range ({$this->startRow} - {$this->endRow})");
        } elseif ($this->onlyExistingCells && !($this->subject->cellExistsByColumnAndRow($this->columnIndex, $row))) {
            throw new Exception('In "IterateOnlyExistingCells" mode and Cell does not exist');
        }
        $this->position = $row;
    ...
    public function prev()
    {
        if ($this->position <= $this->startRow) {
            throw new Exception("Row is already at the beginning of range ({$this->startRow} - {$this->endRow})");
        }
    ...
    protected function adjustForExistingOnlyRange()
    {
        if ($this->onlyExistingCells) {
            ...
            if ($this->startRow > $this->endRow) {
                throw new Exception('No cells exist within the specified range');
            }
